...
Frontend permission integration layer responsible for connecting backend `permissions` array to the frontend permission checking system. Includes loading permissions from `/init/` endpoint into Vuex, route guard page permission checks, and component-level button/feature permission checks.
