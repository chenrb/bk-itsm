...
1. **用户创建**：管理员通过后台或 API 创建用户，设置初始密码、所属部门、直属上级
2. **用户信息维护**：管理员编辑用户的姓名、邮箱、手机号、部门、上级等属性
3. **用户启用/停用**：管理员停用用户后，该用户无法登录；其工单处理权限自动移除，但历史数据保留
4. **密码修改**：用户自助修改密码（需验证旧密码），或管理员重置密码
5. **用户资料兼容**：`get_property()` 方法兼容历史代码中通过 KV 表查询用户属性的调用模式

### User Lifecycle

```
创建(管理员) → 启用 → 正常使用 ← → 密码修改(自助/管理员重置)
                     ↓
                   停用(管理员) → 无法登录，角色/组成员关系保留但不生效
```

### Integration Points

- **认证模块**：User 是 Django `AUTH_USER_MODEL`，Session 和 JWT 认证均依赖此模型
- **部门模块**：User.department 关联主部门，DeptMembership 记录多部门归属
- **角色模块**：User 通过 Role.members 和 UserGroup.members 关联权限和处理人组
- **工单模块**：ticket creator、processor、follower 均引用 User.username
- **init 接口**：`/init/` 返回 `user.chname`、`user.username` 等信息

### Key Behaviors

- 停用用户（`is_active=False`）时：已有 session 不自动失效（依赖 session 自然过期），但该用户无法发起新 session；该用户仍保留在 Role/UserGroup 的 members 中，但在工单处理人解析时应被过滤
- 删除用户为软删除（`is_deleted`），不物理删除，保留外键完整性
- `display_name` 优先级：`nickname` > `chname` > `username`
- `leader` 字段用于工单处理人类型 `STARTER_LEADER` 和 `ASSIGN_LEADER` 的解析
