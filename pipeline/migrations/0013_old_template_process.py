...
def forward_func(apps, schema_editor):
    ...
    TemplateCurrentVersion = apps.get_model(
        'pipeline', 'TemplateCurrentVersion')
    db_alias = schema_editor.connection.alias
    template_list = PipelineTemplate.objects.using(
        db_alias).filter(is_deleted=False)

    for template in template_list:
        TemplateRelationship.objects.using(db_alias).filter(
            ancestor_template_id=template.template_id).delete()
        ...
        versions = TemplateVersion.objects.using(db_alias).filter(
            template_id=template.id).order_by('-id')
        if not (versions and versions[0].md5 == template.snapshot.md5sum):
            TemplateVersion.objects.create(
                template=template,
                snapshot=template.snapshot,
                md5=template.snapshot.md5sum)
